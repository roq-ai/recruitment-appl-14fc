...
import { InterviewListPage as ShowCaseList } from 'pages/interviews';

function DashboardPage() {
  const { session } = useSession();

  const candidateStat = useSWR<PaginatedInterface<CandidateInterface>>(
    () => `/candidates`,
    () => getCandidates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Candidates',
      result: candidateStat?.data?.totalCount,
