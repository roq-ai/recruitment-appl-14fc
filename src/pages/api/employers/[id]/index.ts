...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.employer
    .withAuthorization({
      roqUserId,
  ...
  async function getEmployerById() {
    const data = await prisma.employer.findFirst(convertQueryToPrismaUtil(req.query, 'employer'));
    return res.status(200).json(data);
  }

  async function updateEmployerById() {
  ...
  async function deleteEmployerById() {
    await notificationHandlerMiddleware(req, req.query.id as string);
    const data = await prisma.employer.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
