...
type ColumnType = ColumnDef<EmployerInterface, unknown>;

interface EmployerListPageProps {
...
export function EmployerListPage(props: EmployerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EmployerInterface>>(
    () => `/employers?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/employers/view/${record.id}`} passHref legacyBehavior>
                <Button
